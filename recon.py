...
'''Usage: python3 recon_domain.py --install  -----> Installs tools and dependencies
          ...
          Slack Channels to be created #nmap #takeover #random
'''
__author__      = "Rahul R"
...
# 	with open('output/'+domain+'-unique.txt','r') as http:
# 		for url in http.readlines():
# 			req = url.strip()
# 			# print req
# 			dirsearchCmd = "python3 {} -u {} -e * --plain-text-report={}-dirsearch.txt".format(os.path.join(script_path,'bin/dirsearch/dirsearch.py'),req,domain)
# 			print("\n\033[1;31mRunning Command: \033[1;37m {}".format(dirsearchCmd))
# 			os.system(dirsearchCmd)
# 	time.sleep(1)
...
def nmap():
    ...
    nmap_file.write('\n'.join(urls))
    nmap_file.close()
    nmapCmd= 'nmap -Pn -iL '+output_base+'-nmap-list.txt -oG '+output_base+'-nmap-result'
    os.system(nmapCmd)
    unwanted=['/open/tcp//']
    ...
    with open(output_base+'-unique.txt','r') as g:
    	for url in g.readlines():
    		...
    		try:
    			...
    			print ("Error")
    	slack.chat.post_message("#random","``` ".join(dom)+" ```\n")
    time.sleep(1)
